...
ratings.matrix <- matrix(
  nrow = length(freq.user$ID),
  ncol = length(freq.book$ISBN),
  dimnames = list(freq.user$ID, freq.book$ISBN)
)



for (i in 1:dim(ratings)[1]){
  ratings.matrix[toString(ratings[i,1]), ratings[i,2]] <- ratings[i, 3]
}


real.rating.matrix <- as(ratings.matrix, "realRatingMatrix")
rm(ratings.matrix)
